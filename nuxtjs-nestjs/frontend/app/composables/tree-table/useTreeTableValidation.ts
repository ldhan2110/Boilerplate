...
export interface UseTreeTableValidationOptions {
  rows: Ref<any[]>
...
export interface UseTreeTableValidationReturn {
...
export function useTreeTableValidation(options: UseTreeTableValidationOptions): UseTreeTableValidationReturn {
  const {
    displayedRows,
    columnState,
    visibleColumns,
  ...
  function hasError(row: any, field: string): boolean {
    return (errorMap.value.get(row[rowKey])?.get(field)?.length ?? 0) > 0
  }
