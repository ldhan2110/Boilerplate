...
export interface UseTreeTableFooterOptions {
  rows: Ref<any[]>
...
export interface TreeTableFooterValue {
  value: number
  formatted: string
}

export interface UseTreeTableFooterReturn {
  footerValues: Ref<Record<string, TreeTableFooterValue>>
...
export function useTreeTableFooter(options: UseTreeTableFooterOptions): UseTreeTableFooterReturn {
  const { rows, visibleColumns, footerAggregations, showFooter } = options

  const footerValues = computed<Record<string, TreeTableFooterValue>>(() => {
    if (!showFooter.value) return {}

    const result: Record<string, TreeTableFooterValue> = {}
